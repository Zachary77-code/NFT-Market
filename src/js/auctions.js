Auctions = {
    init: async () => {
        ...
        // const basicNft = await ethers.getContract("NumbersNFT", deployer)
    },
            ...
            .then((instance) => {
                ...
                allContractEvents.get((error, data) => {
                    ...
                    data.forEach((eventData) => {
                        ...
                        if (
                            eventData.event == "AuctionSuccessful" ||
                            eventData.event == "AuctionCancelled"
                        ) {
                            const auctionDisplayed = $("#auctionsRow").find(
                                `.panel-body[data-auction-id=${auctionId}]`
                            )
                            // remove auction
                            console.log(
                                `removing auction ${auctionId} (NFT ${tokenId}) [${eventData.event}]`
                            )
                            auctionDisplayed.parent().parent().remove()
                        } else {
                            // eventData.event == 'AuctionCreated'
                            console.log(
                                `found auction ${auctionId} (NFT ${tokenId}) [${eventData.event}]`
                            )
                            ...
                            auctionTemplate
                                ...
                                .text(web3.fromWei(startingPrice, "ether"))
                            auctionTemplate
                                .find(".panel-body .price-end")
                                .text(web3.fromWei(endingPrice, "ether"))
        ...
        $(document).on("click", "#auctionsRow .view-auction", function (event) {
            ...
            App.contracts.NFTDutchAuction.deployed()
                ...
                .then((o) => {
                    ...
                    $("#viewAuctionModal .price-current").text(
                        "~ " + Math.round(data.currentPrice * 100) / 100
                    )
