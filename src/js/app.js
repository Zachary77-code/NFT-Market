App = {
    ...
    */
    ...
    initContracts: () => {
        ...
        // soldTokens = []
        // console.log(`localStorage.getItem("soldNFTs"): ${localStorage.getItem("soldNFTs")}`)
        // if (
        //     JSON.parse(localStorage.getItem("soldNFTs")) == null ||
        //     JSON.parse(localStorage.getItem("soldNFTs")).length == 0
        // ) {
        //     console.log("hhhhhhhhhhhhhhhhhh")
        //     localStorage.setItem("soldNFTs", JSON.stringify([]))
        //     console.log(`localStorage.getItem("soldNFTs"): ${localStorage.getItem("soldNFTs")}`)
        // }
    ...
    search: () => {
        ...
        if (range < 0) {
            ...
            App.displayErrorMessage(
                "Warning: The amount of numbers you selected is probably to much for your browser"
            )
        } else {
    ...
    mintNFT: (event) => {
        ...
        window.web3.eth.getAccounts((error, accounts) => {
            if (error) {
                ...
                console.log(`contracts te: ${App.contracts.NumbersNFT}`)
                // contract.mint(account, tokenToBuy, { from: account })
                App.contracts.NumbersNFT.deployed()
                    .then((contract) => {
                        ...
                        return contract._mint(account, tokenToBuy, { from: account })
                    })
                    .then((result) => {
